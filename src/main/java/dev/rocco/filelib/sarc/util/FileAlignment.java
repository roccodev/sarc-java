...
public class FileAlignment {
    ...
    public static void writeAligned(File rootDir, FileWriter writer) throws IOException {
        ...
        for(File file : files) {
            ...
            int hash = SarcNode.calculateHash(pathName, pathName.length(), 0x65);
        ...
        for(File file : files) {
            ...
            for(int i = 0; i < padding; i++) {
                writer.writeByte(currentNameOffset += 1, (byte) 0);
            }
        }
        ...
        for(int i = 0; i < files.size(); i++) {
            ...
            if(i + 1 != files.size()) {
                ...
                for(int j = 0; j < padding; j++) {
                    writer.writeByte(currentDataOffset += 1, (byte) 0);
                }
            }
        }
